...
import { projectMedia } from '../content/media.mjs';
...
function renderOverview(locale, project) {
  const c = copy[locale];
  const product = productProfile(project, locale);
  return `<section id="overview" class="case-section product-overview"><p class="eyebrow">${c.productLabel}</p><h2>${c.overview}</h2><p>${escapeHtml(product.context)}</p><h3>${c.workflowLabel}</h3><ol class="product-workflow">${product.workflow.map((step, index) => `<li><span aria-hidden="true">${String(index + 1).padStart(2, '0')}</span><div><h4>${escapeHtml(step.title)}</h4><p>${escapeHtml(step.detail)}</p></div></li>`).join('')}</ol></section>${renderProductScreens(locale, project)}`;
}

/** @param {Locale} locale @param {Project} project */
function renderProductScreens(locale, project) {
  const media = projectMedia.find((item) => item.project === project.slug);
  if (!media) return '';
  const { c, asset } = helpers(locale, project.slug);
  return `<section class="product-screens case-section" id="product-screens"><p class="eyebrow">${c.screensLabel}</p><h2>${c.screensTitle}</h2><div class="screen-gallery ${media.images.length === 1 ? 'screen-gallery-wide' : ''}">${media.images.map((screen) => `<figure><a class="screen-image-link" href="${asset(screen.src)}" target="_blank" rel="noopener" aria-label="${escapeHtml(`${screen[locale].title} — ${c.fullImageLabel}`)}"><img src="${asset(screen.src)}" width="${screen.width}" height="${screen.height}" alt="${escapeHtml(screen[locale].alt)}" loading="lazy" decoding="async"></a><figcaption><strong>${escapeHtml(screen[locale].title)}</strong><p>${escapeHtml(screen[locale].caption)}</p></figcaption></figure>`).join('')}</div></section>`;
}

/** @param {Locale} locale @param {Project} project */
function renderEvidence(locale, project) {
  const evidence = implementationEvidence.find((item) => item.project === project.slug)?.[locale];
  if (!evidence) return '';
  const c = copy[locale];
  return `<div class="implementation-evidence" id="implementation-evidence"><h3>${c.sourceEvidenceTitle}</h3><p>${escapeHtml(evidence.context)}</p><dl>${evidence.references.map((reference) => `<dt><code>${escapeHtml(reference.path)}</code></dt><dd>${escapeHtml(reference.detail)}</dd>`).join('')}</dl><p class="evidence-boundary">${escapeHtml(evidence.boundary)}</p></div>`;
}
/** @param {Locale} locale @param {Project} project */
